...
if (!pin) return;
if (pin === state.pin) {
// También cargamos configuración (IVU, nombre taller, logo) para acceso por PIN
...
try {
// SOLO orderBy -> nada de where para evitar índices extra
...
if (d.tipoTrabajo === "hojalatería" || d.tipoTrabajo === "hojalateria") hojalateria++;
...
const logoUrl = state.config?.logoUrl || "";

let topY = 12;

// Intentar dibujar logo si es base64 (data URL) — no rompe si falla
try {
if (logoUrl && logoUrl.startsWith("data:image")) {
pdf.addImage(logoUrl, "PNG", 14, topY, 24, 24);
}
} catch (e) {
...
pdf.setFontSize(16);
pdf.text(tallerNombre, 105, topY + 6, { align: "center" });

pdf.setFontSize(11);
pdf.text(tallerLinea2, 105, topY + 12, { align: "center" });
if (tallerTelefono) {
pdf.text(`Tel: ${tallerTelefono}`, 105, topY + 18, { align: "center" });
}

pdf.setDrawColor(200);
pdf.line(14, topY + 22, 196, topY + 22);

pdf.setFontSize(13);
pdf.text(titulo, 105, topY + 30, { align: "center" });
}
