...
/* ========== HELPER: COLECCIÓN POR USUARIO ========== */
// Todas las colecciones se guardan bajo:
// users/{uid}/{colección}/...
function colUsuario(nombreColeccion) {
  if (!state.user || !state.user.uid) {
    throw new Error("No hay usuario autenticado en Firebase. Usa Google Auth.");
  }
  return db.collection("users").doc(state.user.uid).collection(nombreColeccion);
}
...
function handlePinLogin(e) {
  ...
  if (!pin) return;
  if (pin === state.pin) {
    // Nota: el PIN SOLO controla la UI, para datos necesitas Google Auth
    showView("appView");
...
async function cargarConfiguracion() {
  if (!state.user) {
    // si no hay usuario, no intentamos leer Firestore
    return;
  }

  try {
    const docRef = colUsuario("ajustes").doc(state.configDocId);
...
async function guardarConfiguracion(e) {
  e.preventDefault();
  if (!state.user) {
    alert("Debes iniciar sesión con Google para guardar configuración.");
    return;
  }
  ...
  try {
    await colUsuario("ajustes").doc(state.configDocId).set(payload, { merge: true });
...
async function guardarIngreso(e) {
  e.preventDefault();
  if (!state.user) {
    alert("Debes iniciar sesión con Google para guardar ingresos.");
    return;
  }
  ...
  try {
    await colUsuario("ingresos").add(data);
...
async function cargarIngresos() {
  ...
  if (!state.user) {
    cont.innerHTML = "<p class='placeholder'>Inicia sesión con Google para ver ingresos.</p>";
  ...
  try {
    const snap = await colUsuario("ingresos")
...
function actualizarKpisIngresos(total, mecanica, hojalateria, pintura, ultimaFecha) {
  ...
  const k4 = qs("#kpiHojalateria?.kpi-value"); // por si no existe en tu HTML
...
function abrirEntrega(ingresoId) {
  openDialog({
    ...
    onSubmit: async (values) => {
      if (!state.user) {
        alert("Debes iniciar sesión con Google para registrar entregas.");
        return;
      }
      ...
      await colUsuario("ingresos").doc(ingresoId).update({
        estado: "entregado",
        entregaNotas: notas,
        entregaAt: firebase.firestore.FieldValue.serverTimestamp()
      });

      const ingresoSnap = await colUsuario("ingresos").doc(ingresoId).get();
      ...
      await colUsuario("facturas").add(facturaData);
...
async function cargarEntregas() {
  ...
  if (!state.user) {
    cont.innerHTML = "<p class='placeholder'>Inicia sesión con Google para ver entregas.</p>";
    return;
  }
...
async function cargarFacturas() {
  ...
  if (!state.user) {
    cont.innerHTML = "<p class='placeholder'>Inicia sesión con Google para ver facturas.</p>";
    return;
  }
  ...
  try {
    const snap = await colUsuario("facturas")
      .orderBy("fecha", "desc")
      .limit(200)
      .get();
    ...
    qsa("[data-marcar-pagada]", cont).forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        await colUsuario("facturas").doc(id).update({ estado: "pagada" });
        cargarFacturas();
        cargarResumenFacturas();
      });
    });
...
async function cargarResumenFacturas() {
  ...
  if (!state.user) {
    cont.innerHTML = "<span class='status-msg'>Inicia sesión con Google para ver el resumen.</span>";
    return;
  }
  ...
  try {
    const snap = await colUsuario("facturas").get();
    let totalPagadas = 0;
...
async function generarFacturaPDF(facturaId) {
  if (!state.user) {
    alert("Debes iniciar sesión con Google para generar PDFs de facturas.");
    return;
  }

  try {
    const snap = await colUsuario("facturas").doc(facturaId).get();
    if (!snap.exists) {
...
async function exportarFacturasListado() {
  if (!state.user) {
    alert("Debes iniciar sesión con Google para exportar el listado de facturas.");
    return;
  }

  try {
    const snap = await colUsuario("facturas").orderBy("fecha", "desc").limit(200).get();
    if (snap.empty) {
...
async function cargarCotizaciones() {
  ...
  if (!state.user) {
    cont.innerHTML = "<p class='placeholder'>Inicia sesión con Google para ver cotizaciones.</p>";
    return;
  }
  ...
  try {
    const snap = await colUsuario("cotizaciones")
      .orderBy("fecha", "desc")
      .limit(200)
      .get();
...
async function nuevaCotizacion() {
  if (!state.user) {
    alert("Debes iniciar sesión con Google para crear cotizaciones.");
    return;
  }
  ...
  try {
    ...
    if (!state.preciosCache.length) {
      const snap = await colUsuario("precios").orderBy("categoria").get();
    ...
    openDialog({
      ...
      onSubmit: async (values) => {
        ...
        await colUsuario("cotizaciones").add(data);
        cargarCotizaciones();
      }
    });
  } catch (err) {
...
async function generarCotizacionPDF(cotizacionId) {
  if (!state.user) {
    alert("Debes iniciar sesión con Google para generar PDFs de cotizaciones.");
    return;
  }

  try {
    const snap = await colUsuario("cotizaciones").doc(cotizacionId).get();
    if (!snap.exists) {
...
async function exportarCotizacionesListado() {
  if (!state.user) {
    alert("Debes iniciar sesión con Google para exportar el listado de cotizaciones.");
    return;
  }

  try {
    const snap = await colUsuario("cotizaciones").orderBy("fecha", "desc").limit(200).get();
    if (snap.empty) {
...
async function cargarPrecios() {
  ...
  if (!state.user) {
    cont.innerHTML = "<p class='placeholder'>Inicia sesión con Google para ver la lista de precios.</p>";
    return;
  }
  ...
  try {
    const snap = await colUsuario("precios")
      .orderBy("categoria")
      .get();
...
function nuevoPrecio() {
  if (!state.user) {
    alert("Debes iniciar sesión con Google para añadir precios.");
    return;
  }
...
async function cargarClientes(busqueda = "") {
  ...
  if (!state.user) {
    cont.innerHTML = "<p class='placeholder'>Inicia sesión con Google para ver clientes.</p>";
    return;
  }
  ...
  try {
    const snap = await colUsuario("clientes")
      .orderBy("nombre")
      .limit(200)
      .get();
...
function nuevoCliente() {
  if (!state.user) {
    alert("Debes iniciar sesión con Google para añadir clientes.");
    return;
  }
...
async function cargarSuplidores(busqueda = "") {
  ...
  if (!state.user) {
    cont.innerHTML = "<p class='placeholder'>Inicia sesión con Google para ver suplidores.</p>";
    return;
  }
  ...
  try {
    const snap = await colUsuario("suplidores")
      .orderBy("nombre")
      .limit(200)
      .get();
...
function nuevoSuplidor() {
  if (!state.user) {
    alert("Debes iniciar sesión con Google para añadir suplidores.");
    return;
  }
  ...
  openDialog({
    ...
    onSubmit: async (values) => {
      await colUsuario("suplidores").add({
        nombre:   values.nombre   || "",
        telefono: values.telefono || "",
        tipo:     values.tipo     || "",
        notas:    values.notas    || "",
...
async function buscarHistorialCliente() {
  ...
  if (!state.user) {
    if (cont) cont.innerHTML = "<p class='placeholder'>Inicia sesión con Google para buscar en el historial.</p>";
    return;
  }
  ...
  try {
    const [ingSnap, facSnap] = await Promise.all([
      colUsuario("ingresos").orderBy("createdAt", "desc").limit(200).get(),
      colUsuario("facturas").orderBy("fecha", "desc").limit(200).get()
    ]);
