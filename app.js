...
const state = {
  ...
  ivuRate: 0.115,               // 11.5% por defecto (decimal)
...
async function cargarConfiguracion() {
  try {
    ...
    // IVU almacenado como porcentaje (ej. 11.5) -> decimal (0.115)
...
/* -------- HEADER PDF PROFESIONAL (CON LOGO) -------- */

function buildPdfHeader(pdf, titulo) {
  const tallerNombre   = (state.config?.tallerNombre || "Nexus Auto Pro 2026").trim();
  const tallerDireccion= state.config?.tallerDireccion || "Taller automotriz · Puerto Rico";
  const tallerTelefono = state.config?.tallerTelefono || "";
  const tallerEmail    = state.config?.tallerEmail || "";

  // Logo (recomendado: pegar Base64 data:image/png en logoUrl)
  if (state.config?.logoUrl && state.config.logoUrl.startsWith("data:image")) {
    try {
      pdf.addImage(state.config.logoUrl, "PNG", 14, 10, 22, 22);
    } catch (e) {
  ...
  pdf.setFontSize(10);
  pdf.text(tallerDireccion, 105, 22, { align: "center" });

  const contacto = [];
  if (tallerTelefono) contacto.push(`Tel: ${tallerTelefono}`);
  if (tallerEmail)   contacto.push(tallerEmail);
  if (contacto.length) {
    pdf.text(contacto.join(" · "), 105, 28, { align: "center" });
  }
...
async function generarFacturaPDF(facturaId) {
  try {
    ...
    pdf.text("Detalle de trabajos realizados:", 14, 98);
    ...
    pdf.setFontSize(9);
    pdf.text("Gracias por confiar en nuestro taller.", 105, 280, { align: "center" });
...
async function nuevaCotizacion() {
  try {
    // Cargamos lista de precios si no está en cache
    ...
    openDialog({
      ...
      onSubmit: async (values) => {
        ...
        // Si el usuario pone total manual, manda sobre todo
...
async function generarCotizacionPDF(cotizacionId) {
  try {
    ...
    pdf.setFontSize(9);
    pdf.text("Cotización sujeta a previa inspección del vehículo.", 105, 280, { align: "center" });
